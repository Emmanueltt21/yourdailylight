...
class SQLiteDbProvider {
  ...
  Future<List<Media>> getAllMediaBookmarks() async {
    ...
    List<Map> results = await db!
        .query("${Media.BOOKMARKS_TABLE}", columns: Media.bookmarkscolumns);
    List<Media> medialist = [];
    results.forEach((result) {
      Media media = Media.fromMap(result as Map<String, dynamic>);
      medialist.add(media);
    });
    //print(categories.length);
    return medialist;
...
//playlists crud
  ...
  Future<List<Hymns>> getAllBookmarkedHymns() async {
    final db = await database;
    List<Map> results = await db!
        .query("${Hymns.BOOKMARKS_TABLE}", columns: Hymns.bookmarkscolumns);
    List<Hymns> medialist = [];
    results.forEach((result) {
      Hymns media = Hymns.fromMap(result as Map<String, dynamic>);
      medialist.add(media);
    });
    //print(categories.length);
    return medialist;
  ...
  Future<List<Versions>> getAllBibleVersions() async {
    ...
    results.forEach((result) {
      Versions versions = Versions.fromMap(result as Map<String, dynamic>);
  ...
  Future<List<Bible>> getAllBible(
    ...
    results.forEach((result) {
      Bible bible = Bible.fromMap(result as Map<String, dynamic>);
      bibleList.add(bible);
    });
    //print(categories.length);
    return bibleList;
  ...
  Future<List<Bible>> getAllColoredVerses() async {
    ...
    List<Bible> bibleList = [];
    results.forEach((result) {
      Bible bible = Bible.fromCOloredMap(result as Map<String, dynamic>);
      bibleList.add(bible);
    });
    //print(categories.length);
    return bibleList;
