...
class PostRepliesModel with ChangeNotifier {
  List<Replies> _items = [];
  bool isError = false;
  int? commentId = 0;
  ...
  PostRepliesModel(BuildContext context, int? commentId, Userdata? userdata,
  ...
  Future<void> fvetchComments() async {
    try {
      final response = await http.post(Uri.parse(ApiUrl.loadpostreplies),
          body: jsonEncode({
  ...
  Future<void> constructComment(String content) async {
    try {
      ...
      print(data.toString());
      final response = await http.post(Uri.parse(ApiUrl.replypostcomment),
  ...
  Future<void> deleteComment(int? commentId, int position) async {
    ...
    try {
      ...
      print(data.toString());
      final response = await http.post(Uri.parse(ApiUrl.deletepostreply),
          body: jsonEncode({"data": data}));
      if (response.statusCode == 200) {
        // If the server did return a 200 OK response,
        // then parse the JSON.
  ...
  Future<void> editComment(int? id, String content, int position) async {
    ...
    try {
      ...
      print(data.toString());
      final response = await http.post(Uri.parse(ApiUrl.editpostreply),
          body: jsonEncode({"data": data}));
      if (response.statusCode == 200) {
        // If the server did return a 200 OK response,
        // then parse the JSON.
